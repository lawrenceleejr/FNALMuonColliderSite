...
CHORDS = []
for CH, ls in zip(TE["chords"][:2], ((0, (1.5, 1.5)), (0, (4, 1.5, 1, 1.5)))):
    az_, D_, dep_ = cc.great_circle_polar(TE["uiuc"], (CH["point"]["lat"], CH["point"]["lon"]))
    CHORDS.append((CH, az_, D_, ls))
...
for k, (nm, E1, E2, _, smeared) in enumerate(STAGES):
    ...
    # the true chords through UIUC South Farms and Green Bay / Lake Winnebago (see two_ends.py)
    for CH, az_, D_, ls in CHORDS:
        ax.plot(az_, rmap(D_), color="w", lw=1.1, ls=ls, alpha=.95, zorder=6)
        ax.plot([az_[-1]], [rmap(D_[-1])], "s", ms=5, color="w", mec="0.1", mew=0.6, zorder=8)
    ax.plot([CHORDS[0][1][0]], [rmap(CHORDS[0][2][0])], "s", ms=5, color="w", mec="0.1", mew=0.6, zorder=8)
    ...
    if k == 0:
        gb, wn = CHORDS[0][0], CHORDS[1][0]
        ax.annotate("UIUC$-$Green Bay chord (dotted)\n%.1f km deep, misses the IP by %.0f km" % (gb["true_chord"]["depth_at_closest_approach_km"],
                    gb["true_chord"]["closest_approach_to_ip_km"]), (CHORDS[0][1][-1], rmap(CHORDS[0][2][-1])), xytext=(8, -2),
                    textcoords="offset points", fontsize=6.0, color="0.1", ha="left", va="top", zorder=30,
                    bbox=dict(boxstyle="round,pad=0.12", fc="w", ec="none", alpha=.8))
        ax.text(0.0, 0.04, "UIUC$-$Winnebago chord (dash-dot):\n%.1f km deep, crosses the site %.1f km W of the IP" % (
                wn["true_chord"]["depth_at_closest_approach_km"], wn["true_chord"]["closest_approach_to_ip_km"]),
                transform=ax.transAxes, fontsize=6.0, color="0.1", ha="left", va="bottom", zorder=30,
                bbox=dict(boxstyle="round,pad=0.12", fc="w", ec="none", alpha=.8))
...
fig.text(0.665, 0.083,
         ...
         "White dotted / dash-dot: the UIUC$-$Green Bay and UIUC$-$Lake Winnebago chords (two_ends.py).",
